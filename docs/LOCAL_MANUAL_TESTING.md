...
This guide explains how to run the **backend API** and **web UI** on your machine for **manual exploratory testing**. Default ports are **4010** (API) and **5180** (web) so they do not clash with common **4000** / **5173** stacks.
...
| --------- | -------------------- | ---------------------------------------------------- |
| `PORT`    | `4010`               | API HTTP port (`[src/server.ts](../src/server.ts)`). |
| `DB_PATH` | `./data/tcms.sqlite` | SQLite file used by the API.                         |


The web app uses **Vite dev server** on **127.0.0.1:5180** and **proxies** `/graphql`, `/health`, and `/api` to the API (see `[apps/web/vite.config.ts](../apps/web/vite.config.ts)`). Override proxy target with `VITE_API_PROXY_TARGET` if the API listens elsewhere.

**Run linked automation** needs a separate sandbox stack (`plan-automation.sqlite`, ports **4012** / **5182**) — see `[docs/plans/plan-automation-local.md](plans/plan-automation-local.md)`.
...
- **Web UI:** [http://127.0.0.1:5180/](http://127.0.0.1:5180/)
- **GraphQL Explorer (browser):** [http://127.0.0.1:5180/graphql](http://127.0.0.1:5180/graphql) — proxied to the API; open in a browser for the interactive UI (`[docs/GRAPHQL_EXPLORER.md](GRAPHQL_EXPLORER.md)`).

Direct API (no Vite): [http://127.0.0.1:4010/graphql](http://127.0.0.1:4010/graphql), [http://127.0.0.1:4010/health](http://127.0.0.1:4010/health).
...
| --------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| `EADDRINUSE` on 4010 / 5180 | Another process uses the port; stop it or change `PORT` / Vite port in config.                                     |
| Web UI cannot reach API     | Ensure API is running; for dev, keep **proxy** and API on `127.0.0.1:4010` unless you set `VITE_API_PROXY_TARGET`. |
